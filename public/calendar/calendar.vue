  ...
  <div class="dateCon">
    ...
    <div v-if="isShow" class="dateBody">
      ...
      <ul class="dateTitle clearfix">
        <li class=" dateWeeks">日</li>
        <li class=" dateWeeks">一</li>
        <li class=" dateWeeks">二</li>
        <li class=" dateWeeks">三</li>
        <li class=" dateWeeks">四</li>
        <li class=" dateWeeks">五</li>
        <li class=" dateWeeks">六</li>
      </ul>
      <!--具体日期-->
      <ul class="dateApp  clearfix">
        <li v-for="(val,index) in date" :style="styleCla(index,val)" @click="choseDay(val)" :class="classCla(val)" class=" dateAppLis">{{isToday(val)}}</li>
      </ul>
      <!--遮罩------日期间隔不能超过30天-->
...
<script>
  ...
  export default{
    ...
    mounted(){
      ...
      //this.setData();
      console.log(this.$data);
      this.$parent.geiTitleList()
    },
    watch:{
      fromDay:function () {
        if(this.fromDay){
          this.startTimeSet(this.fromDay)
          console.log(this.fromDay)
        }
      },
      toDay:function () {
          this.endTimeSet(this.toDay)
